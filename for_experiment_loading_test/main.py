...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="PluginManager runner for Multi-Machine Experiments"
    )
    parser.add_argument(
        "-d", "--dir",
        default="volatile",
        help="Parent directory path for experiments (default: volatile)"
    )
    parser.add_argument(
        "-r", "--ref-time",
        type=int,
        default=None,
        help="Reference time as UNIX seconds (default: 2024-11-13 00:00:00+09:00)"
    )
    parser.add_argument(
        "-R", "--reload",
        action="store_true",
        help="Reload experiments before starting the manager"
    )
    parser.add_argument(
        "-s", "--reload-step",
        type=int,
        default=None,
        help="Step to reload up to (implies --reload)"
    )
    parser.add_argument(
        "-i", "--interval",
        type=int,
    ...
    parser.add_argument(
        "--module-path",
        default="experimental_setting/",
        help="Path for plugin modules (default: experimental_setting/)"
    )
    parser.add_argument(
        "--mode-path",
        default="mode",
        help="Path for mode control files (default: mode)"
    )
    return parser.parse_args()


def main():
    # 2024-11-13 00:00:00 in JST
    UNIX_2024_11_13_00_00_00_IN_JP = 1731423600

    args = parse_args()
    ref_unix = args.ref_time if args.ref_time is not None else UNIX_2024_11_13_00_00_00_IN_JP

    # Experiments の reference_time は minutes 単位を想定
    experiments = Experiments(
        parent_dir_path=Path(args.dir),
        reference_time=ref_unix // 60
    )
    ...
    plugin_manager = PluginManager(
        ...
        mode_path=Path(args.mode_path)
    )

    # リロード指定があれば実行（--reload-step が指定されていれば暗黙に --reload）
    if args.reload or (args.reload_step is not None):
        try:
            step = args.reload_step if args.reload_step is not None else None
        ...
        # リロード後の Experiments でマネージャーを作り直す
        plugin_manager = PluginManager(
            experiments,
            module_path=Path(args.module_path),
            mode_path=Path(args.mode_path)
    ...
    # 監視間隔を引数から反映
    plugin_manager.run(interval=args.interval)
