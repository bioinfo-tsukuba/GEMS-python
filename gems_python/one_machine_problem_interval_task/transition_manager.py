...
class State(ABC):
    """
    Base class for modelling a stage within an experiment.

    Subclass :class:`State` and implement the following hooks:

    * :meth:`task_generator` — build and return a
      :class:`~gems_python.one_machine_problem_interval_task.task_info.TaskGroup`
      that describes the upcoming work, including penalties and constituent tasks.
    * :meth:`transition_function` — inspect the shared variable history
      (:class:`polars.DataFrame`) and return the name of the next state.
    * :meth:`dummy_output` (optional) — provide simulated measurements to keep
      offline runs moving without hardware feedback.

    See ``tests/test_simulate_one.py`` for a self-contained example featuring
    ``InitState``, ``MeasureState``, and ``FinishState``.
    """
    ...
    def dummy_output(self, df: pl.DataFrame, task_group_id: int, task_id: int) -> pl.DataFrame:
        """
        ダミー出力を返す基礎メソッド。各State派生クラスでoverrideしてシミュレーション用の結果を生成すること。
        未実装の場合は例外を投げてシミュレーション時に気付けるようにする。
        """
...
class Experiment:
    """
    Container for a single finite-state experiment.

    Parameters
    ----------
    experiment_name:
        Human-readable label for logging and persistence.
    states:
        Ordered list of :class:`State` instances that model the workflow.
    current_state_name:
        Name of the state that should execute first.
    shared_variable_history:
        :class:`polars.DataFrame` containing signals shared across states.
    current_task_group:
        Active :class:`~gems_python.one_machine_problem_interval_task.task_info.TaskGroup`.
        Normally assigned automatically via :meth:`generate_task_group_of_the_state`.
    current_state_index:
        Derived index that tracks the active state. Managed internally.
    experiment_uuid:
        Unique identifier persisted alongside saved artefacts.
    """
    experiment_name: str
    states: List[State]
...
class Experiments:
    ...
    def auto_load(self):
        """
        Automatically load experiment results generated by an external executor.

        Expected files
        --------------
        ``experiment_result.json``
            Metadata describing the task update outcome.
        ``result.csv``
            Measurement data associated with the completed task.
        ...
        Minimum fields required for processing::
        ...
        Failure cases set ``task_response`` to ``"error"`` and omit
        ``result_path``.
        """
