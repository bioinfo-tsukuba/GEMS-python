...
type TaskFunction = dyn Fn(&DataFrame) -> Result<(ExperimentOperation, OptimalTiming, PenaltyType), Box<dyn Error>>;
...
impl TaskGenerator {
    pub(crate)fn new(experiment_operation: Box<ExperimentOperation>, processing_time_function: Box<ProcessingTimeFunction>, timing_func: Box<TaskFunction>)
    -> Self{
        Self {
