...
pub(crate) mod one_machine_schedule_solver{
    ...
    /// Output: Scheduled task with "ABSOLUTE" time
    /// Note: The schedule must finish in Unit time(min)
    pub(crate) fn simulated_annealing_scheduler_absolute_naive(tasks: Vec<Task>)-> Vec<ScheduledTask> {
        ...
        let tasks: Vec<Task> = tasks.into_iter().map(|mut task| {
            ...
            task.penalty_type = match task.penalty_type {
                ...
                PenaltyType::CyclicalRestPenaltyWithLinear { start_minute, cycle_minute, ranges, coefficient }
                => {
                    let start_minute = start_minute - current_absolute_time;
                    PenaltyType::CyclicalRestPenaltyWithLinear { start_minute, cycle_minute, ranges, coefficient }
                },
    ...
    /// Input: Task with "RELATIVE" optimal time
    /// Output: Scheduled task with "RELATIVE" time
    pub(crate) fn simulated_annealing_scheduler_relative_naive(tasks: Vec<Task>)-> Vec<ScheduledTask>{
        ...
        impl Anneal for TaskSASchedule {
            ...
            fn anneal(&self, param: &Self::Param, temp: Self::Float) -> Result<Self::Param, argmin::core::Error> {
                ...
                for _ in 0..(temp.floor() as u64 + 1){
                    ...
                    let val = rng.sample(rand::distributions::Uniform::new_inclusive(-12*60, 12*60));
        ...
        let lower_bound: Vec<ScheduleTiming> = tasks.iter().map(|task| (task.optimal_timing-24*60).max(0)).collect();
        let upper_bound: Vec<ScheduleTiming> = tasks.iter().map(|task| (task.optimal_timing+24*60).max(0)).collect();
        ...
        impl Anneal for TaskSASchedule {
            ...
            fn anneal(&self, param: &Self::Param, temp: Self::Float) -> Result<Self::Param, argmin::core::Error> {
                ...
                for _ in 0..(temp.floor() as u64 + 1){
                    ...
                    match &self.task[idx].penalty_type {
                        PenaltyType::CyclicalRestPenalty { start_minute, cycle_minute, ranges }
                        => {
                            // modify previous parameter value at random position `idx` by `val`
                            param_n[idx] += val;
                            ...
                            // check if cycle bounds are violated. If yes, project onto bound.
                            let mut relative_param = (param_n[idx] - start_minute)%cycle_minute;
                            
                            for _ in 0..2 {
                                for (start, end) in ranges {
                                    if *start <= relative_param && relative_param <= *end {
                                        match random(){
                                            true => {
                                                    let diff_1 = *end - relative_param;
                                                    param_n[idx] += 1 + diff_1;
                                                    param_n[idx] = param_n[idx].clamp(self.lower_bound[idx], self.upper_bound[idx]);
                                                    relative_param = (param_n[idx] - start_minute)%cycle_minute;
                                            },
                                            false => {
                                                    let diff_1 = relative_param - *start;
                                                    param_n[idx] -= 1 + diff_1;
                                                    param_n[idx] = param_n[idx].clamp(self.lower_bound[idx], self.upper_bound[idx]);
                                                    relative_param = (param_n[idx] - start_minute)%cycle_minute;
                                            }
    ...
    mod tests {
        ...
        fn test_sa_scheduler() {
            ...
            // Schedule the tasks
            let scheduled_tasks = simulated_annealing_scheduler_absolute(tasks);
